...
def grid_move(current_grid, desired_move, desired_distance):
    if desired_move == 'R':
        for CurDist in range(desired_distance):
            current_grid.append([(current_grid[-1][0]+1), (current_grid[-1][1])])
    elif desired_move == 'L':
        for CurDist in range(desired_distance):
            current_grid.append([(current_grid[-1][0]-1), (current_grid[-1][1])])
    elif desired_move == 'U':
        for CurDist in range(desired_distance):
            current_grid.append([(current_grid[-1][0]), (current_grid[-1][1]+1)])
    elif desired_move == 'D':
        for CurDist in range(desired_distance):
            current_grid.append([(current_grid[-1][0]), (current_grid[-1][1]-1)])
    else:
...
res_set = set(map(tuple, Grid_One)) & set(map(tuple, Grid_Two))
res_list = list(map(list, res_set))

manhat_dist_list = []

for intersect_coord in res_list:
    manhat_dist_list.append(abs(intersect_coord[0])+abs(intersect_coord[1]))
